# Yellowtail 3NO Fall
### Age based simulation - only years with both age and length ###
...
## Investigate Rstrap output for combined sex with AGE & LENGTH
...
#Age length plots - real data vs Von B estimates for male and female

ag_new <- ag %>% select(year, sex, length, age) %>%
  filter(year == c(1996:2001) & age >0)

## Von B values obtained fr0m Dwyer et al. 2003

#Male
vb_m <- sim_vonB(Linf = 49, L0 = 0,
               K = 0.19, log_sd = 0,
               length_group = 2, digits = 0)
vb_m(age = 1:10)
vb_m <- vb_m(age = 1:10)
vb_m <- as.data.frame(vb_m)
vb_m <- vb_m %>% setnames('vb_m', 'vbm_length')
vb_m$age <- c(1:10)
vb_m <- vb_m %>% as.data.table


ag_m <- ag_new %>% filter(sex == "male") %>%
  group_by(age) %>%
  summarise(agm_length = mean(length)) %>%
  as.data.table

al_m <- inner_join(vb_m, ag_m)

plot_ly(data = al_m, x = ~age) %>%
  add_markers(y = ~agm_length, name = "real") %>%
  add_markers(y = ~vbm_length, name = "vonB") %>%
  layout(title = "Male Age Growth",
         xaxis = list(title = "Age"),
         yaxis = list(title = "Length"))

#Female
vb_f <- sim_vonB(Linf = 56, L0 = 0,
                 K = 0.16, log_sd = 0,
                 length_group = 2, digits = 0)
vb_f(age = 1:10)
vb_f <- vb_f(age = 1:10)
vb_f <- as.data.frame(vb_f)
vb_f <- vb_f %>% setnames('vb_f', 'vbf_length')
vb_f$age <- c(1:10)
vb_f <- vb_f %>% as.data.table

ag_f <- ag_new %>% filter(sex == "female") %>%
  group_by(age) %>%
  summarise(agf_length = mean(length)) %>%
  as.data.table

al_f <- inner_join(vb_f, ag_f)

plot_ly(data = al_f, x = ~age) %>%
  add_markers(y = ~agf_length, name = "real") %>%
  add_markers(y = ~vbf_length, name = "vonB") %>%
  layout(title = "Female Age Growth",
         xaxis = list(title = "Age"),
         yaxis = list(title = "Length"))

al_real <- inner_join(ag_m, ag_f)

plot_ly(data = al_real, x = ~age) %>%
  add_markers(y = ~agf_length, name = "female") %>%
  add_markers(y = ~agm_length, name = "male") %>%
  layout(title = "Real Age Growth",
         xaxis = list(title = "Age"),
         yaxis = list(title = "Length"))

al_vonb <- inner_join(vb_m, vb_f)

plot_ly(data = al_vonb, x = ~age) %>%
  add_markers(y = ~vbf_length, name = "female") %>%
  add_markers(y = ~vbm_length, name = "male") %>%
  layout(title = "VonB Age Growth",
         xaxis = list(title = "Age"),
         yaxis = list(title = "Length"))
...
# Using combined sex VonB estimates from Dwyer et al. 2003
