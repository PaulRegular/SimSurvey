# Yellowtail 3NO Fall
### Age based simulation - age estimated from length - full data set ###
...
vb <- sim_vonB(Linf = 56, L0 = 0,
                 K = 0.13, log_sd = 0,
                 length_group = 2, digits = 0)
vb(age = 1:20)
vb <- vb(age = 1:20)
vb <- as.data.frame(vb)
vb <- vb %>% setnames('vb', 'length')
vb$age <- c(1:20)
vb <- vb %>% as.data.table

lf_age <- lf %>%
  mutate(age = case_when(
    between(length, 1, 7) ~ 1,
    between(length, 8, 13) ~ 2,
    between(length, 14, 18) ~ 3,
    between(length, 19, 23) ~ 4,
    between(length, 24, 27) ~ 5,
    between(length, 28, 30) ~ 6,
    between(length, 31, 33) ~ 7,
    between(length, 34, 36) ~ 8,
    between(length, 37, 39) ~ 9,
    between(length, 40, 41) ~ 10,
    between(length, 42, 43) ~ 11,
    between(length, 44, 44) ~ 12,
    between(length, 45, 46) ~ 13,
    between(length, 47, 47) ~ 14,
    between(length, 48, 48) ~ 15,
    between(length, 49, 49) ~ 16,
    between(length, 50, 50) ~ 17,
    between(length, 51, 51) ~ 18,
    between(length, 52, 52) ~ 19,
    between(length, 53, 100) ~ 20,
    ))
