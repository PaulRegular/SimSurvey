...
out_all <- strat.fun(setdet = rv_data$setdet, lf = rv_data$lf, ag = rv_data$ag,
                     ...
                     species = 889, survey.year = c(1995:2005, 2007:2016),
                     season = "spring",  # no age-growth 2006, 2017:2019
...
out_all$strat1$age$abundance$summary %>%
  filter(age == 6, sex %in% c("male", "female")) %>%
...
out <- strat.fun(setdet = rv_data$setdet, lf = rv_data$lf, ag = rv_data$ag,
                 ...
                 season = "spring",  # no age-growth 2006, 2017:2019
                 NAFOdiv = c("3N", "3O"), strat = NULL,
                 sex = ("male"), sep.sex = FALSE, # sexually dimorphic growth
                 length.group = 2, length.weight = NULL,
                 group.by = "length & age",
                 export = NULL, plot.results = FALSE)
...
pop <- sim_abundance(ages = 1:16,
                     years = 1:20,
                     R = sim_R(log_mean = log(160000000),
                               log_sd = 0.7,
                               random_walk = FALSE),
