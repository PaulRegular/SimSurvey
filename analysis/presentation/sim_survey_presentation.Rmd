...
- **_Roughly based simulation on 3Ps Cod_**
...
<dev class = "left">
<!-- There is a bug somewhere...had to contain plotly plots in a dev to get it working -->
```{r}
plot_trend(sim) %>% 
  tight_layout(margin = list(l = 70, r = 0, t = 0, b = 40, pad = 0))
```
</dev>
...
<dev class = "left">
```{r}
plot_ly(data = sim$grid_xy, x = ~x, y = ~y, z = ~depth, type = "heatmap") %>%
...
- Try and figure out the bug that prevents the plotly plots from working in this part of the slideshow (alternate fix to containing it in a device)
- Figure out how to use the Google Font with plotly
- Set-up global layout arguents somehow
- Figure out how to make a sub-title for a slide below main title
- Reverse color ramp in grid figure
- Frame by year for spatial abundance plot (consider fixing one year, animate age / fix age and animate year)
