...
sort(unique(ag_spring$age))    # range of redfish age in the spring
...
out <- strat.fun(setdet = rv_data$setdet, lf = rv_data$lf, ag = rv_data$ag,
                 ...
                 group.by = "length",        # No age-growth data since survey year 2000
...
pop <- sim_abundance(ages = 1:50,
                     years = 1:24,
                     R = sim_R(log_mean = log(9500000),
                               log_sd = 0.7,
                               random_walk = T),
                     Z = sim_Z(log_mean = log(0.55),
                               log_sd = 0.08,
                               phi_age = 0.85,
                               phi_year = 0.9),
