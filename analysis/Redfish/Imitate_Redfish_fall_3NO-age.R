#################### Redfish 3NO fall-Age based simulation ###################################
##SEBASTES MENTELLA,REDFISH,DEEP WATER,#794
...
sort(unique(ag_fall$age))    # range of redfish age in the fall
...
out_all <- strat.fun(setdet = rv_data$setdet, lf = rv_data$lf, ag = rv_data$ag,
                     ...
                     species = 794, survey.year = c(1996:2000),
                     season = "spring",  # no age-growth 2001-2019
...
out_all$strat1$age$abundance$summary %>%
  filter(age == 10, sex %in% c("male", "female")) %>%
...
## Real by age
real_ag <- data.frame(af[af$freq>0])
real_ag  %>%
...
# Min sets per strata for all surveys are 2, set density is 1,
# size of length bins for stratified age sampling is 1 for redfish,
# max number of lengths per set are 250
