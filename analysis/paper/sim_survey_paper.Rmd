...
The package also includes several ploting function for making quick plotly-based [@sievert2018] interactive visuals of the simulated population. The plots output from the below code are combined into Figure \@ref(fig:surface).
...
```{r surface, fig.cap = "- Plots produced by `plot_surface` when supplied an list produced by a) `sim_abundance(ages = 1:20, R = sim_R(mean = 3e+07), Z = sim_Z(mean = 0.5))` and b) `sim_abundance(ages = 1:6, R = sim_R(mean = 1e+10), Z = sim_Z(mean = 0.8))`. The abundance at age matrix (`N`) is displayed by default; the `mat` argument can be used to specify another matrix contained within the supplied list, such as `Z`."}
...
pa <- plot_surface(a, mat = "N", scene = "scene1", showscale = FALSE) %>% 
  add_annotations(x = 0.05, y = 0.9, xref = "paper", yref = "paper", showarrow = FALSE,
                  text = "a)")
...
The `make_grid` function is used to generate a survey grid for use in the `sim_distribution` function. The output from `make_grid` is a raster object [@hijmans2016] with four layers: `depth`, `cell`, `division` and `strat`. If a more detailed and realistic grid is required, users can manually generate their own survey grid using real data and this grid can be supplied to `sim_distribution` if the same structure is used. The settings used to make the grid used in our case study are shown in Table \@ref(tab:par) and these settings are also the defaults. Below we generate and plot (combined in Figure \@ref(fig:grid)) a default grid and another grid with a more linear gradient in depth.

```{r, echo = TRUE, eval = FALSE}
a <- make_grid()
b <- make_grid(shelf_depth = 500, shelf_width = 0, depth_range = c(0, 1000))
plot_grid(a)
plot_grid(b)
```


```{r grid, fig.cap = "- Plots produced by `plot_grid` when supplied an raster object produced by a) `make_grid()` and b) `make_grid(shelf_depth = 500, shelf_width = 0, depth_range = c(0, 1000))`."}

a <- make_grid() %>% 
  plot_grid(showscale = FALSE) %>% 
  add_annotations(x = 0, y = 1, xref = "paper", yref = "paper", showarrow = FALSE,
                  text = "a)")
b <- make_grid(shelf_depth = 500, shelf_width = 0, depth_range = c(0, 1000)) %>% 
  plot_grid(showscale = FALSE) %>% 
  add_annotations(x = 0.01, y = 1, xref = "paper", yref = "paper", showarrow = FALSE,
                  text = "b)")
p <- subplot(a, b)

export_plot(p, file = "analysis/paper/figures/plot_grid.png", width = 800, height = 460)
knitr::include_graphics("figures/plot_grid.png")

```


With a population simulated

, and the `sim_distribution` function distributes the population simulated above using `sim_abundance` through the grid:


```{r, echo = TRUE, eval = FALSE}
g <- make_grid(x_range = c(-140, 140),
               y_range = c(-140, 140),
               res = c(3.5, 3.5),
               shelf_depth = 200,
               shelf_width = 100,
...
This function retains all the data simulated by `sim_abundance` and adds a `data.table` with abundance split by age, year and cell (named `sp_N`). Again, note the use of the closures `sim_ays_covar` and `sim_parabola` to impose the age-year-space covariance and the relationship with depth, respectively.
...
The function `sim_survey` can be used to simulate data from one survey over a population simulated using `sim_distribution`:

```{r, echo = TRUE, eval = FALSE}
s <- sim_survey(d, n_sims = 1,
...
Again, this function retains all the objects listed in `d` and adds objects that detail the set locations (`setdet`) and sampling details (`samp`).
...
| `+                     K = 0.1,`                        |                                                Growth rate |                        $K$ |
| `+                     log_sd = 0.1,`                   |      Standard devation of the von Bertalanffy growth curve |                 $\sigma_l$ |
