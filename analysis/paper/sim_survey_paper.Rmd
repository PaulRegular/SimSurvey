...
The construction of the SimSurvey package was motivated by a regional need for assessing the sampling design of trawl surveys conducted along the Newfoundland and Labrador shelf by Fisheries and Oceans Canada. As a tangible first step, we focused our efforts on emulating data from the survey of cod in NAFO division 3Ps since it is one of the most dynamic and heavily sampled stocks in the region. Below we outline the equations and SimSurvey functions used to simulate survey data with similar features to data obtained by the survey of 3Ps cod. The settings were iteratively modified until it was difficult to distinguish the simulated data from the real survey data (see Table \@ref(tab:par) for parameter settings and descriptions). Here we evaluate the efficacy of alternate sampling protocol by assessing deviation between stratified estimates of abundance [@Smith1981] from the true abundance available to the survey. While assessing model-based analyses was an option, we focused on the design-based stratified analysis for simplicity and because of its widespread use.
...
Abundance is simulated using the `sim_abundance` function and the below code was used to simulate the case study population:

```{r, echo = TRUE, eval = FALSE}
set.seed(438) # set seed for reproducible results
n <- sim_abundance(ages = 1:20,
                   years = 1:20,
...
The resultant object, `n`, is a list with data such as the abundance at age matrix (named `N`). Note that the nested functions `sim_R`, `sim_Z` and `sim_vonB` are actually "closures" (i.e. functions that contain data and return functions). These closures avoid the repeated specifications of ages and years and also allows users to use their own closures with a similar structure but different underlying formula.
...
The `make_grid` function is used to generate the grid used in the case study, and the `sim_distribution` function distributes the population, `n`, simulated above using `sim_abundance` through the grid:
