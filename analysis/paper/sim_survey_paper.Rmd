...
p <- subplot(a_plots, b_plots) %>% 
  ...
  add_labels(text = rep(c("Year 1", "Year 2", "Year 3"), 2), x = c(rep(0.005, 3), rep(0.54, 3)), y = c(0.08, 0.5, 0.92), xshift = -20, textangle = -90) %>% 
...
```{r survey, fig.cap = "- Bubble plots and histograms of set catches produced by `plot_survey` when supplied survey data simulated using `sim_survey` under relatively a) high and b) low sampling effort. Note that the first simulation of the survey in the first year is depicted here."}

plot_a <- plot_survey(a, which_sim = 1, which_year = 1)
plot_b <- plot_survey(b, which_sim = 1, which_year = 1)

p <- subplot(plot_a, plot_b, nrows = 2) %>% 
  add_labels(text = "a)", x = 0, y = 1, yshift = 20, xshift = -20) %>% 
  add_labels(text = "b)", x = 0, y = 0.5, yshift = 20, xshift = -20)

export_plot(p, file = "analysis/paper/figures/plot_survey.png", width = 500, height = 600)
knitr::include_graphics("figures/plot_survey.png")

```


- You need to fix up the aspect ratios and margins in the plot_survey subplot
- Mention the `n_sims` argument, mention RAM limitations, and then move to `sim_survey_parallel`
