...
d <- d[d$sim %in% 1:1000 & d$year == which_year & d$set_den %in% which_set_dens &
...
p <- ggplot(data = d) + 
  geom_line(aes(x = age, y = I_hat, color = sim, group = sim), size = 0.1) +
  geom_line(aes(x = age, y = I), color = viridis::viridis(1), size = 0.5) +
  scale_color_viridis(begin = 0.5, guide = FALSE) +
...
d <- merge(sim$surveys, sim$age_strat_error, by = "survey")
d <- d[d$sim %in% 1:1000 & d$age == which_age & d$set_den %in% which_set_dens &
         d$lengths_cap %in% which_len_caps & d$ages_cap == 10, ]

p <- ggplot(data = d) + 
  geom_line(aes(x = year, y = I_hat, color = sim, group = sim), size = 0.1) +
  geom_line(aes(x = year, y = I), color = viridis::viridis(1), size = 0.5) +
  scale_color_viridis(begin = 0.5, guide = FALSE) +
  facet_grid(set_den ~ lengths_cap) +
  xlab("Year") + ylab("Abundance index") +
  theme_minimal() +
