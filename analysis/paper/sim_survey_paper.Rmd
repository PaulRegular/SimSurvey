...
```{r est-fit, cache = TRUE, fig.height = 8, fig.width = 7, fig.cap = "- Stratified estimates of abundance at age across years from 10 simulations of a survey with a set density of 0.002 sets / km^2^ and sampling caps of 500 lengths per set and 10 ages per length group. The heavy dark line indicates the true tred in the population available to the survey."}

which_survey <- 98

d <- sim$age_strat_error
d <- d[d$survey == which_survey & d$sim %in% 1:10, ]
d$age[d$age > 8] <- 8 # aggregate beyond 10
d$age <- as.character(d$age)
d$age[d$age == "8"] <- "8+"
d$age <- factor(d$age, levels = c(as.character(1:7), "8+"))
d <- d[, list(I_hat = sum(I_hat), I = sum(I)), by = c("year", "age", "sim")]

p <- ggplot(data = d) + 
  geom_line(aes(x = year, y = I_hat, color = sim, group = sim), size = 0.1) +
  geom_line(aes(x = year, y = I), color = viridis::viridis(1), size = 0.7) +
  scale_color_viridis(begin = 0.5, guide = FALSE) +
  facet_wrap(~age, scales = "free_y", dir = "v", nrow = 4) +
...
cowplot::save_plot(plot = p, filename = "analysis/paper/figures/true_vs_est.pdf",
...
```{r error-fig, cache = TRUE, fig.height = 3, fig.width = 8.5, results = "hide", fig.cap = "- Wireframe plot of root-mean-squared error (RMSE) from an array of surveys with different sampling protocol. Panels represent surveys with different set densities ($D_{sets}$), x-axes represnet the maximum sampling effort of lengths per set ($M_{lengths}$), and y-axes represent the maximum number of ages to collect per length group ($M_{ages}$)"}
...
```{r age-fig, fig.width = 7, fig.height = 6}
...
ints <- lapply(quants, function(q) {
  d[, list(prob = paste0(q, "%"),
    ...
    by = c("year", "age", "set_den", "lengths_cap")]
})
ints <- rbindlist(ints)
...
ggplot(data = ints) + 
  geom_ribbon(aes(x = age, ymin = lower, ymax = upper, fill = prob, alpha = prob)) +
  geom_line(aes(x = age, y = I), data = d, color = cols[1], size = 1) +
  scale_fill_viridis(name = "Pr", discrete = TRUE, begin = 0.1) +
  scale_alpha_discrete(name = "Pr") + 
  facet_grid(set_den ~ lengths_cap) +
  xlab("Age") + ylab("Abundance index") +
  theme_minimal() +
  scale_y_continuous(labels = format_si()) +
  scale_x_continuous(expand = c(0.01, 0), 
                     labels = c("2", "4", "6", "8+"), 
...
```{r year-fig, fig.width = 7, fig.height = 6}
...
cols <- viridis::viridis(3)
ggplot(data = ints) + 
  geom_ribbon(aes(x = year, ymin = lower, ymax = upper, fill = prob, alpha = prob)) +
  geom_line(aes(x = year, y = I), data = d, color = cols[1], size = 1) +
  scale_fill_viridis(name = "Pr", discrete = TRUE, begin = 0.1) +
  scale_alpha_discrete(name = "Pr") + 
  facet_grid(set_den ~ lengths_cap) +
  xlab("Year") + ylab("Abundance index") +
...
```{r tot-fig, fig.width = 7, fig.height = 3}
...
- Fix facet labels (make them look like the wireframe plot)
- Add RMSE label to colorkey in wireframe plot
- Change vis_sim estimates plot to fan plots like used here
