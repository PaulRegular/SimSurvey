...
## Explore Witch flounder sampling data for the spring season in Div 2J3KL
...
strat_polys <- strat_polys[strat_polys$DIV %in% c("2J", "3K", "3L"), ] # subset to 2J3KL
...
grid <- make_grid(x_range = c(-313, 313),
                  y_range = c(-313, 313),
                  res = c(3.5, 3.5),
                  shelf_depth = 100,
                  shelf_width = 25,
                  depth_range = c(0, 2050),
                  n_div = 3,
...
all_depths <- all_depths %>% mutate(grid = factor(ifelse(strat > 200, "real", "sim")))
