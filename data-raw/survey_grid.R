...
strat_polys <- readOGR("data-raw/DFO_NL_survey_strat/2HJ3KLNOP_Strata_Polygons_WGS84.shp", layer = "2HJ3KLNOP_Strata_Polygons_WGS84")
...
# bathy <- raster::crop(bathy, extent(strat_polys))
# bathy_utm <- raster::projectRaster(bathy, crs = utm_proj)
# r <- raster(extent(bathy_utm), res = c(1, 1), crs = utm_proj)
# bathy_utm <- resample(bathy_utm, r, method = "bilinear") # interpolate to 1 x 1 km utm grid
# bathy <-  raster::projectRaster(bathy_utm, crs = proj4string(strat_polys))
# save(bathy, bathy_utm, file = "data-raw/GEBCO_derived_bathy/GEBCO_derived_bathy.Rdata")
...
survey_grid$depth <- extract(bathy_utm, survey_grid, fun = mean)
