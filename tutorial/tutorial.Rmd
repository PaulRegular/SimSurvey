...
## Simulate population
set.seed(271)
pop <- sim_abundance(ages = seq(1, 10), years = seq(1, 20)) %>%
...
grid_dat <- as_tibble(select(pop$grid_xy, x, y, depth)) %>% distinct()
...
pred <- predict(fit, newdata = grid_dat, return_tmb_object = TRUE, area = cell_area / tow_area)
